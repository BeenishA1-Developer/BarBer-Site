...
const AIStylist: React.FC = () => {
  ...
  const handleConsult = async (e: React.FormEvent) => {
    e.preventDefault();
    const fullInput = selectedStyle ? `${selectedStyle} - ${input}` : input;
    if (!fullInput.trim()) {
      alert('Please select a style or describe your preferences');
      return;
    }
    setIsLoading(true);
    try {
      const advice = await getStyleAdvice(fullInput);
      setResult(advice);
    } catch (error) {
      setResult('Unable to generate recommendation at this time. Please try again.');
    }
    setIsLoading(false);
  };
        ...
        <div className="max-w-6xl mx-auto reveal">
          <div className="flex flex-col lg:flex-row items-stretch bg-[#0F0F0F] border border-white/5 shadow-2xl overflow-hidden">
            <div className="lg:w-1/2 p-12 md:p-24 border-r border-white/5">
              ...
              <p className="text-gray-400 font-light mb-12 text-base leading-relaxed max-w-md">
              ...
              <form onSubmit={handleConsult} className="space-y-8">
                {/* Style Selection */}
                <div className="group">
                  <label className="text-[10px] tracking-[0.3em] uppercase font-bold text-gray-500 group-focus-within:text-accent transition-colors block mb-4">Select Base Style</label>
                  <div className="grid grid-cols-2 gap-2">
                    {styleOptions.map((style) => (
                      <button
                        ...
                        className={`py-2 px-4 text-[10px] tracking-[0.2em] uppercase font-bold border transition-all ${
                          selectedStyle === style
                            ? 'bg-accent text-black border-accent'
                            : 'bg-transparent text-gray-400 border-white/10 hover:border-accent'
                        }`}
                      >
                        {style}
                      </button>
                ...
                <div className="relative group">
                  ...
                  <textarea 
                    className="w-full bg-transparent border-b border-white/10 py-4 text-white placeholder-gray-700 focus:outline-none focus:border-accent transition-all text-sm font-light min-h-[100px] resize-none"
                    placeholder="Hair type, face shape, lifestyle, skin tone..."
                    value={input}
                ...
                <button 
                  type="submit"
                  disabled={isLoading}
                  className="relative group w-full bg-white text-black px-12 py-5 text-[11px] font-bold tracking-[0.3em] uppercase overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg transition-all"
                >
            ...
            <div className="lg:w-1/2 bg-black/40 backdrop-blur-3xl p-12 md:p-24 flex flex-col justify-center relative">
              ...
              {result ? (
                <div className="animate-fade-in">
                  ...
                  <p className="text-2xl md:text-3xl font-light italic leading-relaxed text-gray-100 mb-8">
                    {result}
                  </p>
                  <button
                    onClick={() => {
                    ...
                    className="text-accent text-[10px] tracking-[0.2em] uppercase font-bold hover:text-white transition-colors"
                  >
                    ← New Analysis
                  </button>
                ...
                <div className="text-center opacity-40">
                  <div className="mb-8 inline-block p-10 border border-white/10 rounded-full">
